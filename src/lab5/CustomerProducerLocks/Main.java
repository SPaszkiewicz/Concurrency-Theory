...
public class Main {
    ...
    public static void manyCustomerManyProducerManyBuffor (int opsTime, int waitTime) {
        Bakery bakery = new Bakery(bufforSize, opsTime);
...
//        new Thread(biggerProducer).start();
        ...
        for (int i = 0; i < customerNumber; i++) {
            Producent producer = new Producent(bakery, foodPortion, i, waitTime);
            producerThreads.add(producer);
            new Thread(producer).start();
        }
        for (int i = 0; i < producentNumber; i++) {
            Customer customer = new Customer(bakery, foodPortion, i, waitTime);
